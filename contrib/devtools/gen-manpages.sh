...
LITECOIND=${BITCOIND:-$BINDIR/earthcoind}
LITECOINCLI=${BITCOINCLI:-$BINDIR/earthcoin-cli}
LITECOINTX=${BITCOINTX:-$BINDIR/earthcoin-tx}
LITECOINQT=${BITCOINQT:-$BINDIR/qt/earthcoin-qt}

[ ! -x $LITECOIND ] && echo "$LITECOIND not found or not executable." && exit 1

# The autodetected version git tag can screw up manpage output a little bit
EACVER=($($LITECOINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
